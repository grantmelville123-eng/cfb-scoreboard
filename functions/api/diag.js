...
export async function onRequest(context) {
  ...
  const hasOddsKey = !!env.THE_ODDS_API_KEY;
  const url = new URL(context.request.url);
  return url.searchParams.has("json") ? jsonReport(env, hasOddsKey) : htmlShell(hasOddsKey);
}

/* ───────────── JSON report ───────────── */

async function jsonReport(env, hasOddsKey) {
  ...
  async function pingOdds() {
    ...
    const u = `https://api.the-odds-api.com/v4/sports/?apiKey=${encodeURIComponent(env.THE_ODDS_API_KEY)}`;
    try {
  ...
  const report = {
    ...
    hint: !hasOddsKey
      ? "Odds key missing. Add THE_ODDS_API_KEY in Cloudflare Pages → Settings → Variables and Secrets, then trigger a NEW deployment. Everything except the live betting line works without it."
...
function htmlShell(hasOddsKey) {
  ...
  const envNote  = hasOddsKey
    ...
    : "Not visible. Add it in Cloudflare → Settings → Variables and Secrets, then trigger a new deployment.";
